...
function formatShortDateTime(value) { if (!value) return '-'; const d = new Date(value); return <Box component="span" sx={{ display: 'inline-flex', flexDirection: 'column', lineHeight: 1.25 }}><span>{d.toLocaleDateString('tr-TR')}</span><span>{d.toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}</span></Box>; }
...
function Dashboard({ summary, onFindingsFilter }) {
  ...
  const cards = [
    ['Toplam Bulgu', d.total, 'Tüm kayıtlar', CARD_ACCENTS.total, {}],
    ['Açık Kalan', d.open, 'Kapatılmamış kayıtlar', CARD_ACCENTS.open, { status: 'Devam Ediyor' }],
    ['Kapatılan', d.closed, 'Tamamlanan kayıtlar', CARD_ACCENTS.closed, { status: 'Kapatıldı' }],
    ['Acil', d.severity_counts?.Acil || 0, 'Acil seviye', CARD_ACCENTS.urgent, { severity: 'Acil' }],
  ...
  return <Stack spacing={3}><SectionHeader title="Dashboard" />
    ...
    <Box sx={gridSx(420)}><DataTable title="Geciken Bulgular" rows={d.overdue_findings || []} columns={[{ key: 'record_no', label: 'Kayıt No' }, { key: 'title', label: 'Başlık' }, { key: 'severity', label: 'Seviye', render: (r) => <SeverityChip severity={r.severity} /> }, { key: 'active_due_date', label: 'Termin', render: (r) => formatDay(r.active_due_date) }]} /><DataTable title="Termin Yaklaşanlar" rows={d.approaching_due || []} columns={[{ key: 'record_no', label: 'Kayıt No' }, { key: 'title', label: 'Başlık' }, { key: 'severity', label: 'Seviye', render: (r) => <SeverityChip severity={r.severity} /> }, { key: 'active_due_date', label: 'Termin', render: (r) => formatDay(r.active_due_date) }]} /></Box>
  </Stack>;
}
...
function MsrcPage({ show, refreshApp }) {
  const [months, setMonths] = useState([]); const [selectedMonth, setSelectedMonth] = useState(''); const [summary, setSummary] = useState({}); const [rows, setRows] = useState([]); const [filters, setFilters] = useState({ search: '', cve: '', severity: '', product: '', kb: '', exploited: '', publicly_disclosed: '' }); const [loading, setLoading] = useState(false); const [detail, setDetail] = useState(null);
  const activeFilters = useMemo(() => ({ ...filters, month: selectedMonth }), [filters, selectedMonth]);
  ...
  useEffect(() => { (async () => { try { const res = await getMsrcMonths(); setMonths(res.items || []); const month = res.default || res.items?.[0] || ''; setSelectedMonth(month); await load({ ...filters, month }); } catch (e) { show(e.message, 'error'); } })(); }, []);
  useEffect(() => { if (selectedMonth) load(activeFilters).catch((e) => show(e.message, 'error')); }, [selectedMonth, filters.search, filters.cve, filters.severity, filters.product, filters.kb, filters.exploited, filters.publicly_disclosed]);
  const setFilter = (key, value) => setFilters((f) => ({ ...f, [key]: value }));
  const sync = async () => { try { setLoading(true); const res = await syncMsrcMonth(selectedMonth); show(`${res.message || 'MSRC senkronizasyonu tamamlandı.'} Yeni: ${res.created || 0}, Güncellenen: ${res.updated || 0}`, res.status === 'empty' ? 'warning' : 'success'); await load(activeFilters); } catch (e) { show(e.message, 'error'); } finally { setLoading(false); } };
  ...
  const cards = [['Toplam CVE', summary.total_cve, CARD_ACCENTS.total], ['Critical', summary.critical, CARD_ACCENTS.critical], ['High / Important', summary.high_important, CARD_ACCENTS.high], ['Medium / Moderate', summary.moderate, CARD_ACCENTS.medium], ['Low', summary.low, CARD_ACCENTS.low], ['Exploited', summary.exploited, CARD_ACCENTS.urgent], ['Publicly Disclosed', summary.publicly_disclosed, CARD_ACCENTS.info], ['KB Sayısı', summary.kb_count, CARD_ACCENTS.open]];
  return <Stack spacing={2.5} sx={{ fontFamily: 'Segoe UI, Arial, Helvetica, sans-serif' }}><SectionHeader title="Microsoft CVE / MSRC" description="Microsoft Security Update Guide / CVRF API verilerini seçilen aya göre çekin, filtreleyin ve bulguya dönüştürün." />
    <Box sx={gridSx(170)}>{cards.map(([label, value, accent]) => <SummaryCard key={label} label={label} value={value || 0} accent={accent} />)}</Box>
    ...
    <Paper sx={panelSx}><Grid container spacing={1.5}>{[['search', 'Arama'], ['cve', 'CVE'], ['product', 'Product'], ['kb', 'KB']].map(([key, label]) => <Grid item xs={12} md={key === 'search' ? 3 : 2} key={key}><TextField fullWidth size="small" label={label} value={filters[key]} onChange={(e) => setFilter(key, e.target.value)} /></Grid>)}<Grid item xs={12} md={1.5}><FormControl size="small" fullWidth><InputLabel>Severity</InputLabel><Select label="Severity" value={filters.severity} onChange={(e) => setFilter('severity', e.target.value)}><MenuItem value="">Tümü</MenuItem>{['Critical', 'Important', 'High', 'Moderate', 'Medium', 'Low'].map((x) => <MenuItem key={x} value={x}>{x}</MenuItem>)}</Select></FormControl></Grid><Grid item xs={12} md={1.5}><FormControl size="small" fullWidth><InputLabel>Exploited</InputLabel><Select label="Exploited" value={filters.exploited} onChange={(e) => setFilter('exploited', e.target.value)}><MenuItem value="">Tümü</MenuItem><MenuItem value="true">Evet</MenuItem><MenuItem value="false">Hayır</MenuItem></Select></FormControl></Grid><Grid item xs={12} md={2}><FormControl size="small" fullWidth><InputLabel>Publicly Disclosed</InputLabel><Select label="Publicly Disclosed" value={filters.publicly_disclosed} onChange={(e) => setFilter('publicly_disclosed', e.target.value)}><MenuItem value="">Tümü</MenuItem><MenuItem value="true">Evet</MenuItem><MenuItem value="false">Hayır</MenuItem></Select></FormControl></Grid></Grid></Paper>
    <Paper sx={{ ...panelSx, overflow: 'hidden' }}><TableContainer sx={{ maxWidth: '100%', overflowX: 'auto' }}><Table size="small" sx={{ minWidth: 1380, tableLayout: 'fixed', '& th': { bgcolor: '#f8fafc', fontWeight: 800, whiteSpace: 'nowrap' }, '& td': { verticalAlign: 'middle', overflow: 'hidden' }, '& tbody tr': { cursor: 'pointer' }, '& tbody tr:hover td': { bgcolor: '#eef6ff' } }}><TableHead><TableRow>{['CVE', 'Başlık', 'Severity', 'Product', 'KB', 'Impact', 'Exploited', 'Publicly Disclosed', 'Release Month', 'Release Date', 'İşlem'].map((h) => <TableCell key={h}>{h}</TableCell>)}</TableRow></TableHead><TableBody>{rows.length ? rows.map((row) => <TableRow hover key={row.id} onClick={() => setDetail(row)}><TableCell sx={{ width: 130 }}><Typography fontWeight={800}>{row.cve_id}</Typography></TableCell><TableCell sx={{ width: 240 }}>{truncateText(row.title)}</TableCell><TableCell sx={{ width: 120 }}><SeverityChip severity={row.severity || row.max_severity} /></TableCell><TableCell sx={{ width: 230 }}>{truncateText(row.product)}</TableCell><TableCell sx={{ width: 105 }}>{row.kb_article || '-'}</TableCell><TableCell sx={{ width: 240 }}>{truncateText(row.impact)}</TableCell><TableCell sx={{ width: 100 }}><Chip size="small" label={boolText(row.exploited)} color={row.exploited ? 'error' : 'default'} /></TableCell><TableCell sx={{ width: 145 }}><Chip size="small" label={boolText(row.publicly_disclosed)} color={row.publicly_disclosed ? 'warning' : 'default'} /></TableCell><TableCell sx={{ width: 130 }}>{row.release_month || '-'}</TableCell><TableCell sx={{ width: 125 }}>{formatDay(row.release_date)}</TableCell><TableCell sx={{ width: 150 }} onClick={(e) => e.stopPropagation()}><Stack direction="row" spacing={1}><Button size="small" onClick={() => setDetail(row)}>Detay</Button><Button size="small" variant="outlined" onClick={() => convert(row)}>Bulguya Dönüştür</Button></Stack></TableCell></TableRow>) : <TableRow><TableCell colSpan={11} align="center">Kayıt yok. Ay seçip “MSRC Verisini Çek” butonunu kullanabilirsiniz.</TableCell></TableRow>}</TableBody></Table></TableContainer></Paper>
    <MsrcDetail row={detail} onClose={() => setDetail(null)} onConvert={convert} />
  </Stack>;
}

function MsrcDetail({ row, onClose, onConvert }) {
  const raw = row?.raw || (() => { try { return JSON.parse(row?.raw_json || '{}'); } catch { return {}; } })();
  return <Dialog open={Boolean(row)} onClose={onClose} maxWidth="lg" fullWidth><DialogTitle>MSRC CVE Detayı - {row?.cve_id}</DialogTitle><DialogContent dividers><Stack spacing={2}>{row && <><Box sx={gridSx(240)}><DetailField label="CVE" value={row.cve_id} /><DetailField label="Başlık" value={row.title} /><DetailField label="Severity"><SeverityChip severity={row.severity || row.max_severity} /></DetailField><DetailField label="Product" value={row.product} /><DetailField label="KB" value={row.kb_article} /><DetailField label="Fixed Build" value={row.fixed_build} /><DetailField label="Exploited" value={boolText(row.exploited)} /><DetailField label="Publicly Disclosed" value={boolText(row.publicly_disclosed)} /><DetailField label="Release Date" value={formatDay(row.release_date)} /></Box><DetailField label="Impact" value={row.impact} /><DetailField label="Microsoft URL">{row.url ? <Button href={row.url} target="_blank" rel="noreferrer">Microsoft bağlantısını aç</Button> : '-'}</DetailField><DetailField label="Ham veri özet alanı"><Box component="pre" sx={{ m: 0, p: 1.5, bgcolor: '#f8fafc', borderRadius: 1, maxHeight: 260, overflow: 'auto', fontSize: 12 }}>{JSON.stringify(raw, null, 2).slice(0, 5000)}</Box></DetailField></>}</Stack></DialogContent><DialogActions><Button onClick={() => onConvert(row)} disabled={!row}>Bulguya Dönüştür</Button><Button onClick={onClose}>Kapat</Button></DialogActions></Dialog>;
}
